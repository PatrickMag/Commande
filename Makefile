...
$(APP).desktop:
	...
	$(Q) echo "Name=$(APP)" >> $@
	$(Q) echo "Exec=sh -c \"mkdir -p \\\$$HOME/.local/share/icons && cp -f \\\$$HOME/$(APP)/Icones/logo.png \\\$$HOME/.local/share/icons/ ; cp \\\$$HOME/$(APP)/$(APP).desktop `xdg-user-dir DESKTOP` ; \\\$$HOME/$(APP)/$(APP)\"" >> $@
	$(Q) echo "Icon=logo.png" >> $@
